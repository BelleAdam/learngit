...
function photoClip(container, option) {
	...
	// $file.attr("accept", "image/jpg,image/jpeg,image/png");
		...
		if (!/image\/\w+/.test(this.files[0].type)) {
			...
			fileReader.onload = function(e) {
				...
				if (kbs > 1) {
					...
					filePicLoad(this.result);
					/*
					var $tempImg = $("<img>").hide();
					// console.log("dasf")
					$tempImg.load(function() {
						// IOS设备中，如果的照片是竖屏拍摄的，虽然实际在网页中显示出的方向也是垂直，但图片数据依然是以横屏方向展示
						...
						EXIF.getData(this, function() {
			        		exifInfo = EXIF.getAllTags(this);
				        	if(exifInfo){
				        		if(exifInfo.Orientation!='' && exifInfo.Orientation!=undefined){
				        			rote = EXIF.getAllTags(this).Orientation;
				        		}
				        	}
				        });
						if (rote==6 || sourceWidth == realityHeight) {
							angleOffset = 90;
						}else if(rote==8){
							angleOffset = 90;
						}else if(rote==3){}
	...
	window.filePicLoad=function(url,app){
		var $tempImg = $("<img>").hide();
			// console.log("dasf")
			$tempImg.load(function() {
				// IOS设备中，如果的照片是竖屏拍摄的，虽然实际在网页中显示出的方向也是垂直，但图片数据依然是以横屏方向展示
	...
	function initClip() {
		...
		canvas.width = clipWidth*2;
		canvas.height = clipHeight*2;
	}
	function clipImg() {
		if (!imgLoaded) {
			alert("亲，当前没有图片可以裁剪!");
	...
	function compressImg(sourceImgObj, quality, angleOffset, outputFormat){
		...
		var newImageData = cvs.toDataURL(mimeType, 1);
		return newImageData;
	}
